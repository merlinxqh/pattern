...
public class ExcelExportUtil {
	 ...
	 * @throws IOException
	 */
	public static File exportList(List<Map<String,Object>> data, boolean isMap){
		FileOutputStream o = null;
		File file = null;
		try { //创建工作薄
			...
			filepath += ".xlsx";
			file = new File(filepath);
			newReadInRows(workBook, data, isMap);
			o = new FileOutputStream(file);
			workBook.write(o);
...
//		style.setBorderRight(HSSFCellStyle.BORDER_THIN);//右边框
	...
	public static void down(HttpServletRequest request, HttpServletResponse response, File file, String downFileName) {
    	...
    	try {
			...
			if(StringUtils.contains(userAgent, "MSIE")){//IE浏览器
				finalFileName = URLEncoder.encode(downFileName,"UTF-8");
			}else {
				finalFileName = new String(downFileName.getBytes("UTF-8"), "ISO8859-1");
			}
			response.setContentType("application/octet-stream");
	...
	public static void newReadInRows(Workbook workBook, List<Map<String,Object>> data, boolean isMap) throws Exception {
		for(int i = 0; i < data.size(); i++) {
			...
			try {
				...
				if (data.get(i) != null && data.get(i).size() > 0) {
					...
					setHeadRow(workBook, sheet, headers);
					setRowData(sheet, 1, properties, (List<Object>) data.get(i).get("data"), isMap);
				}
			} catch (Exception e) {
				logger.error("--" + e.getMessage());
			}
